...
function zoomPan({ srcW, srcH, dur, fps, zmax, cx, cy }) {
  ...
  // Supersample: zoompan rounds its crop window to whole pixels each frame, so a
  // slow zoom visibly "steps"/shimmers. Pre-scaling the source up by SS makes
  // each step SS× smaller → smooth. zoompan then downscales back to srcW×srcH.
  // x,y are in the SS-scaled space, so the focus point scales by SS too.
  const SS = 4;
